...
class TopKDiceLoss(nn.Module):
    def __init__(self, n_classes, top_k_ratio=0.75):
        super().__init__()
        self.n_classes = n_classes
    ...
    def forward(self, inputs, target, mask=None):
        ...
        for i in range(self.n_classes):
            pred_i = inputs[:, i]
            tgt_i = target_onehot[:, i].float()
            if mask is not None:
                m = mask[:, 0].float()
                inter = (pred_i * tgt_i * m).sum()
                union = (pred_i * m).sum() + (tgt_i * m).sum()
            else:
                inter = (pred_i * tgt_i).sum()
                union = pred_i.sum() + tgt_i.sum()
            dice = (2 * inter + 1e-10) / (union + 1e-10)
            class_losses.append(1 - dice)

        class_losses_t = torch.stack(class_losses)
        k = max(1, int(self.n_classes * self.top_k_ratio))
        topk_loss, _ = torch.topk(class_losses_t, k)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets, mask=None):
        ...
        logits:  [B, C, H, W]
        targets: [B, H, W]  long
        mask:    [B, H, W]  float 0/1 or soft weights
        """
        B, C, H, W = logits.shape
        log_prob = F.log_softmax(logits, dim=1)  # [B,C,H,W]
        prob = log_prob.exp()

        targets_clamp = targets.clamp(0, C - 1)
        log_pt = log_prob.gather(1, targets_clamp.unsqueeze(1)).squeeze(1)  # [B,H,W]
        pt = prob.gather(1, targets_clamp.unsqueeze(1)).squeeze(1)
